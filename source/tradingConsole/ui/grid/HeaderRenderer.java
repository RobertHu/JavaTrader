...
public class HeaderRenderer extends DefaultTableCellRenderer
{
					   ...
					   boolean selected, boolean focused, int row, int column)
	{
		...
		{
			DataGrid dataGrid = (DataGrid)table;
			if(dataGrid.get_BindingSource() != null)
			{
				column = dataGrid.get_BindingSource().getColumnByCaption(value.toString());
				if(column == -1)	column = dataGrid.get_BindingSource().getColumnByName(value.toString());
				if(column == -1) return this;
				String caption = dataGrid.get_BindingSource().getPropertyDescriptors()[column].get_Caption();
				setValue(caption);
			}
		}
		else
